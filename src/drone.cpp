...
// Target angles for keyboard control
float targetRoll = 0.0;    // Target roll angle in degrees
float targetPitch = 0.0;   // Target pitch angle in degrees
float targetYaw = 0.0;     // Target yaw rate
...
double KalmanUncertaintyAngleYaw = 15*15;
...
void loop(){
  ...
  double rawYaw = atan2(Mag(1),Mag(0));
  yaw = rawYaw;
  ...
  kalman_1d(kalmanAngleYaw,KalmanUncertaintyAngleYaw,GZ,rawYaw);
  kalmanAngleYaw = Kalman1DOutput[0];
  KalmanUncertaintyAngleYaw = Kalman1DOutput[1];
  
  //PID
  float rollCorrection = Roll.compute(targetRoll * DEG_TO_RAD, kalmanAngleRoll, 0.04);
  float pitchCorrection = Pitch.compute(targetPitch * DEG_TO_RAD, kalmanAnglePitch, 0.04);
  float yawCorrection = Yaw.compute(targetYaw, kalmanAngleYaw, 0.04);
  ...
  if (isArmed && !emergencyStop) {
    ...
    m0 = constrain(m0, minSignal, maxSignal-15);
    m1 = constrain(m1, minSignal, maxSignal-15);
    m2 = constrain(m2, minSignal, maxSignal-15);
    m3 = constrain(m3, minSignal, maxSignal-15);
...
void setupWebServer() {
  ...
  server.on("/api/sensors", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    json += "\"heading\":" + String(kalmanAngleYaw * RAD_TO_DEG , 2) + ",";
  ...
  server.on("/control/angles", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("roll") && request->hasParam("pitch")) {
      targetRoll = request->getParam("roll")->value().toFloat();
      targetPitch = request->getParam("pitch")->value().toFloat();
      targetRoll = constrain(targetRoll, -10.0, 10.0);  // Limit to ±10 degrees
      targetPitch = constrain(targetPitch, -10.0, 10.0);
      Serial.println("Target angles set - Roll: " + String(targetRoll) + "°, Pitch: " + String(targetPitch) + "°");
    }
    request->send(200, "text/plain", "OK");
  });

  server.on("/control/yaw", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("rate")) {
      targetYaw = request->getParam("rate")->value().toFloat();
      targetYaw = constrain(targetYaw, -1.0, 1.0);  // Limit yaw rate
      Serial.println("Target yaw rate set: " + String(targetYaw));
    }
    request->send(200, "text/plain", "OK");
  });
...
<body>
  ...
  <div class="keyboardControls">
    ...
    <div class="controlRow">
      ...
      <div>
        <div class="keyIndicator" id="keyW">W</div>
        <div class="keyIndicator" id="keyA">A</div>
        <div class="keyIndicator" id="keyS">S</div>
        <div class="keyIndicator" id="keyD">D</div>
      </div>
    </div>
    <div class="controlRow">
      <span>Thrust/Yaw:</span>
      <div>
        <div class="keyIndicator" id="keyUp">↑</div>
        <div class="keyIndicator" id="keyLeft">←</div>
        <div class="keyIndicator" id="keyDown">↓</div>
        <div class="keyIndicator" id="keyRight">→</div>
      </div>
    </div>
  </div>
  ...
  <div class="targetDisplay">
    ...
    <div class="targetValue">
      <span>Roll:</span>
      <span id="targetRoll">0.0°</span>
    </div>
    <div class="targetValue">
      <span>Pitch:</span>
      <span id="targetPitch">0.0°</span>
    </div>
    <div class="targetValue">
      <span>Yaw Rate:</span>
      <span id="targetYaw">0.0</span>
    </div>
    <div class="targetValue">
      <span>Throttle:</span>
      <span id="targetThrottle">0%</span>
    </div>
  </div>
  ...
  <script>
    ...
    throttleSlider.oninput = function() {
      currentThrottle = parseFloat(this.value);
    ...
    let currentTargetPitch = 0;
    let currentTargetYaw = 0;
    ...
    function updateTargetDisplay() {
      document.getElementById('targetRoll').innerHTML = currentTargetRoll.toFixed(1) + '°';
      document.getElementById('targetPitch').innerHTML = currentTargetPitch.toFixed(1) + '°';
      document.getElementById('targetYaw').innerHTML = currentTargetYaw.toFixed(1);
      document.getElementById('targetThrottle').innerHTML = currentThrottle.toFixed(0) + '%';
    }
    
    function sendControlCommands() {
      // Send angle commands
      fetch(`/control/angles?roll=${currentTargetRoll}&pitch=${currentTargetPitch}`)
        .catch(error => console.log('Angle control error:', error));
      
      // Send yaw command
      fetch(`/control/yaw?rate=${currentTargetYaw}`)
        .catch(error => console.log('Yaw control error:', error));
      
      // Send throttle command
      fetch(`/throttle?value=${currentThrottle}`)
        .catch(error => console.log('Throttle control error:', error));
    }
    ...
    document.addEventListener('keydown', function(event) {
      ...
      // Only process if not already pressed (prevent key repeat)
      if (keysPressed.has(key) || keysPressed.has(code)) return;
      ...
      switch(key) {
        case 'w':
          currentTargetPitch = -10; // Pitch forward
          updateKeyIndicator('W', true);
          break;
        case 's':
          currentTargetPitch = 10;  // Pitch backward
          updateKeyIndicator('S', true);
          break;
        case 'a':
          currentTargetRoll = -10;  // Roll left
          updateKeyIndicator('A', true);
          break;
        case 'd':
          currentTargetRoll = 10;   // Roll right
          updateKeyIndicator('D', true);
          break;
        case ' ':
          // Spacebar for emergency stop
          emergencyStop();
          return; // Don't process further after emergency stop
      }
      
      // Arrow keys for thrust and yaw
      switch(code) {
        case 'ArrowUp':
          currentThrottle = Math.min(currentThrottle + 10, 100);
          updateKeyIndicator('Up', true);
          break;
        case 'ArrowDown':
          currentThrottle = Math.max(currentThrottle - 10, 0);
          updateKeyIndicator('Down', true);
          break;
        case 'ArrowLeft':
          currentTargetYaw = -0.5; // Yaw left
          updateKeyIndicator('Left', true);
          break;
        case 'ArrowRight':
          currentTargetYaw = 0.5;  // Yaw right
          updateKeyIndicator('Right', true);
          break;
      }
    ...
    document.addEventListener('keyup', function(event) {
      ...
      // Reset to neutral position when keys are released
      switch(key) {
        case 'w':
        case 's':
      ...
      switch(code) {
        case 'ArrowUp':
          updateKeyIndicator('Up', false);
          break;
        case 'ArrowDown':
          updateKeyIndicator('Down', false);
          break;
        case 'ArrowLeft':
        case 'ArrowRight':
          ...
          updateKeyIndicator('Left', false);
          updateKeyIndicator('Right', false);
          break;
      }
    ...
    // Update throttle slider when changed by keyboard
    function syncThrottleSlider() {
      throttleSlider.value = currentThrottle;
      throttleValue.innerHTML = currentThrottle;
    }
    
    // Make sure page is focused for keyboard events
