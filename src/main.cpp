...
KalmanCompFilter KalmanRoll(2); // Uncertainty is expected error of start angle, in this case i expect it to be about 2 degrees.
KalmanCompFilter KalmanPitch(2);
KalmanCompFilter KalmanYaw(42); // Yaw can be anything between -180 to 180 so i insert a random value
...
void loop(){
  ...
  imu.Update();
  double Roll = KalmanRoll.Compute(imu.GetRoll(),imu.GX,DTSeconds);
  double Pitch = KalmanPitch.Compute(imu.GetPitch(),imu.GY,DTSeconds);
  double Yaw = KalmanYaw.Compute(imu.GetYaw(),imu.GZ,DTSeconds);
